...
const normalizeProductVariants = ({ edges }: ProductVariantConnection) => {
    return edges?.map(({ node: { id, selectedOptions, sku, title, priceV2, compareAtPriceV2 } }) => {
        return {
            id,
            name: title,
            sku: sku ?? id,
...
export function normalizeProduct(productNode: ShopifyProduct): Product {
    const { id, title: name, vendor, images, variants, description, descriptionHtml, handle, priceRange, options, ...rest } = productNode;
...
function normalizeLineItem({ node: { id, title, variant, quantity, ...rest } }: CheckoutLineItemEdge): LineItem {
    return {
        id,
