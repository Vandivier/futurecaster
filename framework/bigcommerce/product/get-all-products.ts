...
export type ProductEdge = NonNullable<NonNullable<GetAllProductsQuery['site']['products']['edges']>[0]>;
...
const FIELDS = ['products', 'featuredProducts', 'bestSellingProducts', 'newestProducts'];

export type ProductTypes = 'products' | 'featuredProducts' | 'bestSellingProducts' | 'newestProducts';

export type ProductVariables = { field?: ProductTypes } & Omit<GetAllProductsQueryVariables, ProductTypes | 'hasLocale'>;
...
async function getAllProducts<T extends Record<keyof GetAllProductsResult, any[]>, V = any>(opts: {
    query: string;
    variables?: V;
    config?: BigcommerceConfig;
    preview?: boolean;
...
async function getAllProducts({
    ...
    const { data } = await config.fetch<RecursivePartial<GetAllProductsQuery>>(query, { variables });
