...
const getProducts: ProductsHandlers['getProducts'] = async ({ res, body: { search, category, brand, sort }, config }) => {
    ...
    if (category && Number.isInteger(Number(category))) url.searchParams.set('categories:in', category);

    if (brand && Number.isInteger(Number(brand))) url.searchParams.set('brand_id', brand);
    ...
    const { data } = await config.storeApiFetch<{ data: { id: number }[] }>(url.pathname + url.search);
