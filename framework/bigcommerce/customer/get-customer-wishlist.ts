...
export type WishlistItem = NonNullable<definitions['wishlist_Full']['items']>[0] & {
    product?: ProductEdge['node'];
};

export type GetCustomerWishlistResult<T extends { wishlist?: any } = { wishlist?: Wishlist }> = T;
...
async function getCustomerWishlist<T extends { wishlist?: any }, V = any>(opts: {
    url: string;
    variables: V;
    config?: BigcommerceConfig;
    includeProducts?: boolean;
...
async function getCustomerWishlist({
    ...
    const { data = [] } = await config.storeApiFetch<RecursivePartial<{ data: Wishlist[] }>>(
        `/v3/wishlists?customer_id=${variables.customerId}`
    );
    const wishlist = data[0];

    if (includeProducts && wishlist?.items?.length) {
        const entityIds = wishlist.items?.map((item) => item?.product_id).filter((id): id is number => !!id);
