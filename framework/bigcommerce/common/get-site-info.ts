...
export type CategoriesTree = NonNullable<GetSiteInfoQuery['site']['categoryTree']>;

export type BrandEdge = NonNullable<NonNullable<GetSiteInfoQuery['site']['brands']['edges']>[0]>;
...
async function getSiteInfo<T extends { categories: any[]; brands: any[] }, V = any>(opts: {
    query: string;
    variables?: V;
    config?: BigcommerceConfig;
    preview?: boolean;
...
async function getSiteInfo({
    ...
    const { data } = await config.fetch<RecursivePartial<GetSiteInfoQuery>>(query, { variables });
