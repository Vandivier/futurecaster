...
export type HookFetcherOptions = { method?: string } & ({ query: string; url?: string } | { query?: string; url: string });
...
export type HookFunction<Input extends { [k: string]: unknown } | null, T> = keyof Input extends never
    ? () => T
...
> = {
    ...
    ): HookFunction<Input & { swrOptions?: SwrOptions<Data, FetchInput> }, ResponseState<Data> & State>;
    fetchOptions: HookFetcherOptions;
    fetcher?: HookFetcherFn<Data, FetchInput>;
};

export type SWRHookContext<Data, FetchInput extends { [k: string]: unknown } = {}> = {
    useData(context?: { input?: HookFetchInput | HookSWRInput; swrOptions?: SwrOptions<Data, FetchInput> }): ResponseState<Data>;
};
...
> = {
    useHook(context: MutationHookContext<Data, FetchInput>): HookFunction<Input, HookFunction<ActionInput, Data | Promise<Data>>>;
    fetchOptions: HookFetcherOptions;
    fetcher?: HookFetcherFn<Data, FetchInput>;
};

export type MutationHookContext<Data, FetchInput extends { [k: string]: unknown } | null = {}> = {
...
export type SwrOptions<Data, Input = null, Result = any> = ConfigInterface<Data, CommerceError, HookFetcher<Data, Input, Result>>;
