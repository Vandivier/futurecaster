...
const normalizeProductVariants = (variants: SwellVariant[], productOptions: normalizedProductOption[]) => {
    return variants?.map(({ id, name, price, option_value_ids: optionValueIds = [] }) => {
        const values = name.split(',').map((i) => ({ name: i.trim(), label: i.trim() }));

        const options = optionValueIds.map((id) => {
            const matchingOption = productOptions.find((option) => {
                return option.values.find((value: ProductOptionValue) => value.id == id);
            });
...
export function normalizeProduct(swellProduct: SwellProduct): Product {
    ...
    const productVariants = variants ? normalizeProductVariants(variants, productOptions) : [];
...
function normalizeLineItem({ id, product, price, variant, quantity }: CartLineItem): LineItem {
    const item = {
        id,
        ...
        variant: {
            ...
            name: variant?.name!,
            image: {
                url: product?.images && product.images.length > 0 ? product?.images[0].file.url : '/',
            },
